...
function initBrandShowcase() {
    ...
    // Initialize the brand logos swiper
    const brandSwiper = new Swiper('.brand-logos-swiper', {
        // Basic settings
        loop: true,
        autoplay: {
            delay: 0,
        ...
        // Additional settings for smooth infinite scroll
        allowTouchMove: true,
        grabCursor: true,
...
function initLiquidBackground() {
    ...
    // Colors that complement the green theme
    const colors = ['#059669', '#0D9488', '#34D399']; // Primary green, teal, light green
    ...
    // Create palette texture (exactly from ReactBits)
    ...
    // CommonClass (exactly from ReactBits)
    class CommonClass {
        constructor() {
            this.width = 0;
            this.height = 0;
    ...
    // MouseClass (exactly from ReactBits)
    class MouseClass {
        constructor() {
    ...
    // AutoDriver (exactly from ReactBits)
    class AutoDriver {
    ...
    // All shader code (exactly from ReactBits)
    const face_vert = `
...
`;
    ...
    // ShaderPass (exactly from ReactBits)
    class ShaderPass {
        constructor(props) {
            this.props = props || {};
    ...
    // All shader pass classes (exactly from ReactBits)
    ...
    // Simulation class (exactly from ReactBits)
    class Simulation {
        constructor(options) {
            this.options = {
    ...
    // Output class (exactly from ReactBits)
    class Output {
        constructor() {
            this.init();
        }
    ...
    // WebGLManager (exactly from ReactBits)
    class WebGLManager {
        constructor(props) {
            this.props = props;
    ...
    // Create WebGL manager with options
    ...
    // Apply all options to simulation
    ...
    container._liquidWebGL = webgl;
    container._liquidObserver = observer;
    container._liquidResizeObserver = resizeObserver;

    // Cleanup function
    window.addEventListener('beforeunload', () => {
        if (container._liquidWebGL) {
            container._liquidWebGL.dispose();
        }
        if (container._liquidObserver) {
            container._liquidObserver.disconnect();
        }
        if (container._liquidResizeObserver) {
